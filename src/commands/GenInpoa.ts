...
export const GenInpoa: Command = {
  ...
  async execute(client: Client, interaction: CommandInteraction) {
    ...
    const locomotiveCount =
      locomotivePicker <= 5 ? 2 : locomotivePicker <= 8 ? 3 : 4;
    ...
    embed.addFields({
      name: "Front Powers",
      value: frontLocomotives.join(", ")
    });

    if (middleLocomotives.length > 0)
      embed.addFields({
        name: "Middle DPU",
        value: middleLocomotives.join(", ")
      });
    ...
    embed.addFields({
      name: "Breakdown Chance",
      value: `${breakdownChance.toFixed(1)}%`
    });
...
function generateLocomotive(): [number, number] {
  ...
  if (picker <= 12) {
    ...
    locomotive =
      gevo === 4
        ? Math.floor(Math.random() * (2569 - 2520 + 1)) + 2520
