...
async function getLocomotiveData(locomotiveId: string, tzOffset: string = '0'): Promise<void> {
  ...
  try {
    ...
    const { data, verificationToken, antiForgeryToken } = processResponseData(response);
...
function processResponseData(response: AxiosResponse) {
  ...
  const data: string = responseText.split('locomotiveView.init(')[1].split(', false);')[0];
